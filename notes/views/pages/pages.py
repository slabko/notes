...
@blueprint.route('/pages/edit/<int:page_id>/attachments/', methods=['POST'])
def upload(page_id):
    ...
    uploads_service().save_attachment(page_id, filename, file.stream)
    return flask.redirect(f'/pages/{page_id}/')
...
def read_file(page_id, file_name):
    file_path = uploads_service().get_attachment_path(page_id, file_name)
    mime_type, _ = mimetypes.guess_type(file_name)
    return flask.send_file(file_path, mimetype=mime_type)


@blueprint.route('/pages/edit/<int:page_id>/attachments/<file_name>/delete',
                 methods=['POST'])
def dalete_file(page_id, file_name):
    uploads_service().delete_attachment(page_id, file_name)
    return flask.redirect(f'/pages/{page_id}/')
